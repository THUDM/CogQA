...
def cognitive_graph_propagate(tokenizer, data: 'Json eval(Context as pool)', model, model_cg, device, setting:'distractor / fullwiki' = 'distractor', max_new_nodes = 5):
    ...
    while len(queue) > 0:
        ...
        # pdb.set_trace()
        hop_preds, ans_preds, semantics_preds, no_ans_logits = model(ids, segment_ids, input_mask, sep_positions,
            None, None, None, None, 
...
def main(BERT_MODEL='bert-base-uncased', model_file='./models/bert-base-uncased.bin', data_file='./hotpot_dev_distractor_v1.json', max_new_nodes=5):
    ...
    model_cg.to(device).eval()
    # lp = LineProfiler()
    # lp_wrapper = lp(cognitive_graph_propagate)
    # lp.add_function(BertForMultiHopQuestionAnswering.forward)
    with torch.no_grad():
        for data in tqdm(dataset):
            gold, ans, graph_ret, ans_nodes = cognitive_graph_propagate(tokenizer, data, model, model_cg, device, setting = setting, max_new_nodes=max_new_nodes)
            # gold, ans = lp_wrapper(tokenizer, data, model, model_cg, device, setting = setting, max_new_nodes=max_new_nodes)
